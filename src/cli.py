...
Examples:
  ...
  python -m src.cli simulate --symbols "AAPL,MSFT" --dry-run
    ...
    simulate_parser = subparsers.add_parser(
        'simulate',
        help='Paper trading mode (safe, simulated execution)'
    )
    ...
    simulate_parser.add_argument(
        ...
        type=float,
        default=1.5,
        help='Slippage in basis points (default: 1.5)'
    )
    simulate_parser.add_argument(
        '--commission',
        type=float,
    ...
    backtest_parser = subparsers.add_parser(
        'backtest',
        help='Historical backtesting mode'
    )
    ...
    live_parser = subparsers.add_parser(
        'live',
        help='⚠️  LIVE TRADING MODE (real money at risk)'
    )
...
def add_common_args(parser):
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help='Max open positions (default: 5)'
    )
    parser.add_argument(
        '--sizing-method',
        type=str,
    ...
    parser.add_argument(
        '--dry-run',
        action='store_true',
        help='Dry run mode: validate but don\'t place orders'
    )
    parser.add_argument(
        '--journal-pdf',
        action='store_true',
        help='Generate PDF journal report'
    )
    parser.add_argument(
        '--initial-capital',
        type=float,
