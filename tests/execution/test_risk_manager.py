...
def test_validate_order_no_stop_loss(risk_manager):
    ...
    is_valid, _error = risk_manager.validate_order(
        symbol="AAPL",
        side=OrderSide.BUY,
        quantity=Decimal("100"),
        entry_price=Decimal("100"),
...
def test_max_positions_limit(risk_manager, broker):
    ...
    for i in range(5):
        broker.place_order(
            symbol=f"TEST{i}", side=OrderSide.BUY, quantity=Decimal("10"), order_type="market"
        )
...
def test_consecutive_losses_circuit_breaker(risk_manager):
    ...
    for _ in range(3):
        risk_manager.record_trade_result(pnl=Decimal("-100"), risk_pct=0.01)
...
def test_volatility_limit_check(risk_manager):
    ...
    # Normal volatility
    is_ok, msg = risk_manager.check_volatility_limit(current_atr=2.0, average_atr=2.0)
    assert is_ok
    assert msg is None

    # High volatility (3.5x normal)
    is_ok, msg = risk_manager.check_volatility_limit(current_atr=7.0, average_atr=2.0)
    assert not is_ok
...
def test_closing_order_allowed_when_at_max_positions(risk_manager, broker):
    ...
    for i in range(5):
        broker.place_order(
            symbol=f"TEST{i}", side=OrderSide.BUY, quantity=Decimal("5"), order_type="market"
        )
